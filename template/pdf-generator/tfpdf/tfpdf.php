...
function WriteCenter($h, $txt, $link='')
{
	// Output text in flowing mode
	$cw = &$this->CurrentFont['cw'];
	$w = $this->w-$this->rMargin-$this->x;
	...
	while($i<$nb)
	{
		...
		if($c=="\n")
		{
			// Explicit line break
			if ($this->unifontSubset) {
				$this->Cell($w,$h,mb_substr($s,$j,$i-$j,'UTF-8'),0,2,'C',0,$link);
			}
			else {
				$this->Cell($w,$h,substr($s,$j,$i-$j),0,2,'C',0,$link);
			}
			$i++;
			$sep = -1;
			$j = $i;
		...
		if($l>$wmax)
		{
			...
			{
				if ($this->unifontSubset) {
					$this->Cell($w,$h,mb_substr($s,$j,$sep-$j,'UTF-8'),0,2,'C',0,$link);
				}
				else {
					$this->Cell($w,$h,substr($s,$j,$sep-$j),0,2,'C',0,$link);
				}
				$i = $sep+1;
			}
			$sep = -1;
	...
	if($i!=$j) {
		if ($this->unifontSubset) {
			$this->Cell($l,$h,mb_substr($s,$j,$i-$j,'UTF-8'),0,0,'C',0,$link);
		}
		else {
			$this->Cell($l,$h,substr($s,$j),0,0,'C',0,$link);
		}
	}
}
